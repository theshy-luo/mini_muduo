...
int main()
{
    ...
    if (res == -1)
    {
        LOG_ERROR << "socket bind fail errno is " << errno << "\n";
        return 0;
    }

    // 开始监听
    ::listen(listenFd, SOMAXCONN);
    LOG_DEBUG << "Echo Server listening on port 12347...\n";
    ...
    listenChannel.SetReadCallback([&](Timestamp receive_time) 
    {
        ...
        if (connFd < 0)
        {
            LOG_ERROR << "accept4 fail \n";
            return;
        }
        LOG_DEBUG << "New connection fd=" << connFd
                << " receive_time=" << receive_time.milli_seconds_since_epoch()
                << "\n";
        ...
        connChannel->SetReadCallback(
            [connFd, connChannel, &loop](Timestamp receive_time) 
        {
            char buf[1024];
            ...
            {
                LOG_DEBUG << "Connection closed fd=" << connFd << " receive_time="
