...
namespace mini_muduo 
{
    ...
    {
        // 核心绑定：当 Channel 被 epoll 唤醒时，会通过这些钩子回到 TcpConnection
        // 内部处理
        ...
        LOG_INFO << "TcpConnection::TcpConnection[" << name_ << "] at" << this
                << " fd=" << sockfd;
    }

    TcpConnection::~TcpConnection() 
    {
        LOG_INFO << "TcpConnection::~TcpConnection[" << name_ << "] state=" 
                << state_;
    }
        ...
        if (state_ == kConnected) 
        {
            ...
            if (!channel_->is_writing() && output_buffer_.ReadableBytes() == 0) 
            {
                ...
                else 
                {
                    ...
                    {
                        LOG_ERROR << "TcpConnection::send write error\n";
                    }
            ...
            {
                output_buffer_.Append(static_cast<const char *>(message) + nwrote,
                                    remaining);
                if (!channel_->is_writing()) 
                {
