...
namespace mini_muduo 
{
    ...
    : loop_(loop), name_(name),
      ...
      started_(false), next_conn_id_(0), 
    ...
    void TcpServer::NewConnectionCallback(int sockfd,
                                            const InetAddress &peer_addr)
    {
        InetAddress localaddr;
        socklen_t addrlen = static_cast<socklen_t>(sizeof(*localaddr.GetSockAddr()));
        if (::getsockname(sockfd, (struct sockaddr *)(localaddr.GetSockAddr()),
                        &addrlen) < 0) 
        {
        ...
        connections_.emplace(
            conn_name, TcpConnectionPtr(new TcpConnection(io_loop, 
                conn_name, sockfd, localaddr, peer_addr)));
    ...
    {
        LOG_DEBUG << "TcpServer::RemoveConnection [" << name_ << "] - connection "
            << conn->name() << "\n";
