...
namespace mini_muduo
{
    namespace detail
    {
        ...
        void ReadTimerFd(int timer_fd, Timestamp now)
        {
            ...
            LOG_DEBUG << "TimerQueue::handleRead() " << how_many << " at "
                << now.ToString();
    ...
    : loop_(loop), timer_fd_(detail::CreateTimerFd()),
      timer_fd_channel_(loop, timer_fd_), calling_expired_timers_(false)
    {
    ...
    TimerId TimerQueue::AddTimer(Timestamp when, TimerCallback cb,
        double interval)
    {
        Timer *timer = new Timer(when, cb, interval);
        loop_->RunInLoop(std::bind(&TimerQueue::AddTimeInLoop, this, when, timer));
    ...
    void TimerQueue::CancelTimer(TimerId timer_id)
    {
        ...
        &TimerQueue::CancelTimerInLoop, this, timer_id));
    }
    ...
    std::vector<TimerQueue::Entry> TimerQueue::GetExpired(Timestamp now)
    {
        ...
        for (auto it = timers_.begin(); it != end; ++it)
        {
            ...
            active_timers_.erase(ActiveTimer(it->second, 
                it->second->sequence()));
        }
    ...
    void TimerQueue::Reset(const std::vector<Entry> &expired, Timestamp now)
    {
        for (auto &entry : expired)
        {
            ...
            if (entry.second->repeat() && cancelling_timers_.find(timer) == 
                cancelling_timers_.end())
            {
