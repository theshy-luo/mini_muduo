...
namespace mini_muduo 
{
    ...
    EventLoop::EventLoop()
    : looping_(false), quit_(false), thread_id_(CurrentThread::tid()),
      calling_pending_functors_(false), wakeup_fd_(detail::CreateEventfd()),
      wakeup_channel_(new Channel(this, wakeup_fd_)),
      poller_(new EpollPoller(this)), timer_queue_(new TimerQueue(this)),
      timing_wheel_(nullptr) 
    {
        // 如果发现这个线程里已经有人创建过 EventLoop 了，这是致命的，立刻报错
        if (t_loopInThisThread) 
        {
            LOG_ERROR << "Fatal Error: Another EventLoop " << t_loopInThisThread
        ...
        wakeup_channel_->SetReadCallback(
            std::bind(&EventLoop::WakeUpHandleRead, this));
    ...
    void EventLoop::UpdateChannel(Channel *channel) 
    {
        if (!IsInLoopThread()) 
        {
            LOG_ERROR << "Fatal: UpdateChannel must be called in loop thread!\n";
            return;
        }
