...
namespace mini_muduo 
{
    ...
    void Socket::Listen() 
    {
        ...
        if (res == -1) 
        {
            LOG_ERROR << "socket listen fail errno is " << errno << "\n";
            return;
        }
    }
    ...
    void Socket::SetReuseAddr(bool on) 
    {
        ...
        if (res < 0) 
        {
            LOG_ERROR << "socket setsockopt (SOL_SOCKET, SO_REUSEADDR) fail errno is "
                    << errno << "\n";
            return;
        }
    }
    ...
    void Socket::ShutdownWrite() 
    {
        ...
        if (res < 0) 
        {
            LOG_ERROR << "socket shutdownWrite fail errno is " << errno << "\n";
        }
    }
} // namespace mini_muduo